...
class DBOperation():
    def __init__(self):
        ...
        file.close()
        self.mydb = mysql.connector.connect(host="localhost", user=datadict["username"], passwd=datadict["password"],
    ...
    def InsertOneTimeData(self, spaces_for_two, spaces_for_four):
        cursor = self.mydb.cursor()
        for x in range(spaces_for_two):
            cursor.execute("INSERT into slots (space_for,is_empty) values ('2','1')")
            self.mydb.commit()

        for x in range(spaces_for_four):
